function [tri] = Remove_Bad_Triangles(T,lon,lat,dep,k)
...
for i=1:length(tri)
    ...
    tri_dep=mean([dep(tri(i,1)),dep(tri(i,2)),dep(tri(i,3))]);
    
    cent_tri=[cent_tri;tri_lon,tri_lat,tri_dep];
end

cent_tri_lon=cent_tri(:,1);
cent_tri_lat=cent_tri(:,2);
cent_tri_dep=cent_tri(:,3);
...
[in,on] = inpolygon(cent_tri_lon,cent_tri_lat,lon(k),lat(k));
